...
class ListDocuments extends Component
{
    ...
    public $nama_dokumen, $lampiran, $dokumenModel;
    ...
    public function klasifikasiJenisDokumen()
    {
        $semua_dokumen = Dokumen::all();

        foreach ($semua_dokumen as $dokumen) {
            $search = explode(" ", $dokumen->nama_dokumen);
            $jenis_dokumen_count = JenisDokumen::where('jenis_dokumen', 'like', '%' . $search[0] . '%')->count();

            if ($jenis_dokumen_count > 0) {
                if ($jenis_dokumen_count == 1) {
                    if (!array_key_exists(1, $search)) {
                        $jenis_dokumen = JenisDokumen::where('jenis_dokumen', 'like', '%' . $search[0] . '%')->first();
                    }
                    if (array_key_exists(1, $search)) {
                        $jenis_dokumen = JenisDokumen::where('jenis_dokumen', 'like', '%' . $search[0])->orWhere('jenis_dokumen', 'like', "%{$search[0]} {$search[1]}%")->first();
                    }
                    // $dokumen->jenis_dokumen_id = $jenis_dokumen->id;
                }
                ...
                if ($jenis_dokumen_count > 1) {
                    ...
                    if (array_key_exists(1, $search)) {
                        $jenis_dokumen = JenisDokumen::where('jenis_dokumen', 'like', "%{$search[0]} {$search[1]}%")->first();
                    }
                    // $dokumen->jenis_dokumen_id = $jenis_dokumen->id;
                }
        ...
        session()->flash('message', "Klasifikasi Jenis Dokumen Berhasil !");
    }
